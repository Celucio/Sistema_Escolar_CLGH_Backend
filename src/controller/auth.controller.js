...
const login = async (req, res) => {
  try {
    ...
    if (primerInicioSesion) {
      // Si es el primer inicio de sesión, retorna un mensaje indicando la necesidad de cambio de contraseña
      return res.status(200).json({ mensaje, primerInicioSesion });
    }

    // Si no es el primer inicio de sesión, establece la cookie con el token
    res.cookie('token', token, { maxAge: 3600000, httpOnly: true });

    // Resto del código para manejar un inicio de sesión exitoso
